...
function TooltipProvider({ children, delayDuration = 0 }: { children: React.ReactNode; delayDuration?: number }) {
  return <>{children}</>
}
...
function TooltipTrigger({ children, asChild, ...props }: React.HTMLAttributes<HTMLButtonElement> & { asChild?: boolean }) {
...
function TooltipContent({ 
  ...
  React.useEffect(() => {
    if (!context.open || !context.triggerRef.current || !contentRef.current || hidden) return
